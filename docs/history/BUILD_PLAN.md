...
- [x] **M4.4 — Licensing & tiers (v0.15.0)** (built before M4.2/M4.3 — it gates them): `licensing/` = Ed25519-signed, OFFLINE-verified license tokens (pubkey baked into the build, private key issues; `scripts/license.py keygen|issue`). Three tiers + add-ons (`entitlements.py`, `PRICING.md`): **base** (data plane, 5 MCPs, own client), **plus** (chat UI, 12 MCPs, core agents), **pro** (full app, all agents, unlimited MCPs) + add-ons (slack/discord/extra_mcps/premium_data). Enforcement at the EXISTING seams (`enforce.py`): team roster filtered to entitled agents, MCP group list capped to quota, slack/discord/app CLI gated — ops agents never gated. KEY DESIGN: a source/server build with no baked pubkey is unrestricted (dev uncrippled); only a product build with the pubkey enforces, and a missing/expired/tampered license fails OPEN to free, never locked out. `agents license [--activate KEY]`. Live-verified: keygen→issue pro→verify→entitlements unlock; tamper + expiry + wrong-key rejected; tier gradient free<base<plus<pro. **Distribution: NOT App Store — direct download** (`scripts/build-desktop.sh` → PyInstaller onedir + zip; Gatekeeper signing is the Developer-ID turn-on item). BYO model key (bundled-quota subscription later).
- [x] **M4.2 — Web chat UI (v0.16.0)**: a framework-free SPA (`gateway/ui/`) the gateway serves at `/` — the real chat surface over the existing async-task + SSE flow (POST /message?mode=async → GET /tasks/{id}/events → final result). Live tool-call feed (streams the orchestrator + delegates' tool calls as they fire — fixed: only the TASK `end` marker is terminal, run_end fires per sub-agent), mini-markdown renderer (headings/bold/lists/TABLES with best-price highlighting/_sources_), starter chips, connection status, advisory footer. Mounted behind the SAME chat_ui gate as `agents serve`/`agents app` (StaticFiles at /, API routes win; a free product build serves the API but not the UI — tested). Ships in the package (hatchling includes ui/*). Live-verified: real conversation end-to-end (NBA Finals + platform-tour, tool feed streamed, tables rendered, $0.06). Site: "The app on your machine" preview section (the UI in a window frame), hero → "Now downloadable / Get the app".
