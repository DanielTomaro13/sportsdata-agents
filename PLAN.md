...
| **D28** | Agent harness / runtime ([§8.2](#82-context-engineering--the-agent-harness)) | Build on Pydantic AI / use Claude Managed Agents / custom loop / **one runtime-neutral agent-spec abstraction** | **(Amended at M0.7, 2026-06)** **Custom harness over the litellm `ModelGateway` (cross-provider tool-calling) + `MCPManager`** — not Pydantic AI: by build time the gateway already owned tiers/fallback/budgets/metering and the manager owned scoped tools + deny, so Pydantic AI would have duplicated the model layer and surrendered the §8.2 loop/context control. litellm preserves model-agnosticism (D2/D12). The **agent-spec abstraction stays runtime-neutral**, so Pydantic AI or Claude Managed Agents can still bind behind it later (e.g. Managed Agents for long-running Anthropic sessions). | + Full loop control (stop conditions, context policy, verification), zero duplicated model abstraction, every tested component reused. − We own the loop's correctness; revisit if a framework's maturity outpaces it. |
