...
- [~] **🚪 P0 EXIT GATE:** CI green ✓; audit/trace/cost recording proven (M0.11–M0.12) ✓; grounded+sourced team answers proven live (M0.12–M0.13, OpenRouter) ✓; **odds-math eval PASSES on Groq** (exact 0.4, `verified=True`, via the `final_answer` channel) ✓. **Pending two runs, blocked only by free-tier token arithmetic** (Groq TPD 200k consumed by today's debugging; refills ~24 h):
  1. `pytest -m eval` — the stats-fact case (its last run ended with the HONEST "data unavailable" §13.1 behaviour under provider saturation, not a defect).
  2. `.venv/bin/agents run "Find the best price across bookmakers for an upcoming AFL match this week, and tell me if there is value. Cite books and fetch times." --agent odds_specialist`
  Run both tomorrow (or immediately after an OpenRouter top-up — `--model openrouter/openai/gpt-4o-mini`), then flip to ✅. Compose→Postgres bring-up: documented in README; covered by the planned P1 Postgres CI job.
