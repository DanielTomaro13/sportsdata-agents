...
- [ ] **More market types per book**: TAB `numTopMarkets` already returns Line/Totals propositions; Kambi listView carries Line/Totals offers; PB event details carry every market — extend the existing normalizers' filters (Pinnacle already captures spread+total).
