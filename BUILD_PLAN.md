...
- **All team sports settle — first-party else ESPN** (Daniel's directive): `_mlb_results` (MLB StatsAPI, codedGameState F) joins NBA/AFL/NRL as first-party; everything else reverts to `_espn_results` — one generic `espn_scoreboard` collector over a catalogued league list (NFL, college football/basketball ×2, NHL, WNBA, college baseball, EPL/MLS/UCL/World Cup/A-League; extend by adding rows; the aggregator exclusion was about ODDS — results are facts). Live: MLB 19 finals + ESPN 4 (WNBA/NHL — the leagues in season in June; the rest activate as their seasons start). **Local cron installed** (the interim ops loop until P3's daemon): daily 23:30 `agents resolve` + `agents results`; weekly Mon 09:00 `agents steward` — marker-tagged `# sportsdata-agents-cron` in Daniel's crontab, logs in /tmp/agents-cron.log and /tmp/agents-steward.log.
...
- [ ] **Racing for the AU books** (NEXT: TAB/Sportsbet/Entain/BetR/PointsBet/Unibet all carry racing routes): same Feed recipe, racing domain conventions proven by `fanduel_racing_win` (race=event, runner-number=selection, win=market).
- [ ] **(P3) Scheduled ingestion deployment** — the loop currently runs when invoked; P3 makes it self-operating: daemon/cron for `agents ingest --loop`, the ops/triage agents watching feed failures, `market_monitor` push alerts, and the warehouse moved to Postgres/Timescale with retention (at all-markets volume a laptop SQLite is not the home; ~5-50k snapshots/cycle).
