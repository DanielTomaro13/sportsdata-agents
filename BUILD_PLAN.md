...
- [ ] **Racing for the AU books** (TAB/Sportsbet/Entain/BetR/PointsBet/Unibet all carry racing routes): same Feed recipe, racing domain conventions proven by `fanduel_racing_win` (race=event, runner-number=selection, win=market).
- [ ] **TAB/Sportsbet/PB additional market types**: TAB `numTopMarkets≥3` returns Line/Totals propositions; PB event details carry every market; Sportsbet beyond `primaryMarket` means per-event firehoses — extend filters when wanted (Pinnacle and Kambi already capture spread+total).
